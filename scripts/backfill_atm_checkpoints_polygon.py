...
def fetch_option_1m(option_ticker: str, session: date, api_key: str) -> dict[str, float]:
    ...
    for attempt in range(6):
        ...
        if r.status_code in (401, 403):
            raise RuntimeError(
                f"{r.status_code} for {option_ticker}: plan does not include option minute aggregates"
            )
...
def list_option_contracts(
    underlying: str, session: date, api_key: str, *, contract_type: str = "call"
) -> list[dict]:
    """Listed option contracts with expiration >= session, sorted by expiry then strike."""
...
def resolve_missing_contract(row: dict, session: date, api_key: str) -> None:
    ...
    right = (row.get("right") or "call").strip().lower()
    contract_type = "put" if right == "put" else "call"
    contracts = list_option_contracts(ticker, session, api_key, contract_type=contract_type)
    if not contracts:
        row["notes"] = (row.get("notes") or "") + f"; polygon: no {contract_type} contracts"
        return
